...
const NICK_COLORS = ['light_blue', 'light_red', 'dark_red', 'light_green', 'dark_green', 'magenta', 'light_magenta', 'orange', 'cyan', 'light_cyan'];
...
class Bot {
  ...
  ignoredIrcUser(user) {
    return CommandParser.readJSONFile(CommandParser.IGNORED_USERS_FILE).irc.some(i => i.toLowerCase() === user.toLowerCase());
  }

  ignoredDiscordUser(user) {
    return CommandParser.readJSONFile(CommandParser.IGNORED_USERS_FILE).discord.some(i => i.toLowerCase() === user.toLowerCase());
  }
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        CommandParser.findAdmin(author, function() {
					logger.debug('Sending command message to IRC', ircChannel, text);
          new CommandParser(text).run("irc", this.ircClient, ircChannel);
        });
      } else {
        if (text !== '') {
          // Convert formatting
  ...
  sendToDiscord(author, channel, text) {
    ...
    if (this.isCommandMessage(text)) {
      ...
      CommandParser.findAdmin(author, function() {
				logger.debug('Sending command message to Discord', `#${discordChannel.name}`, text);
				new CommandParser(text).run("discord", discordChannel);
      });
      return;
    }
